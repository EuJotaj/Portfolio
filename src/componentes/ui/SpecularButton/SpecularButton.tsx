...
void main() {
  vec2 p = gl_FragCoord.xy - uCenter; float d = sdRoundedRect(p, uHalfSize, uRadius); vec2 light = vec2(cos(uAngle), sin(uAngle));
  ...
  float line = gaussianLine(d, uThickness); float edge = 1.0 - smoothstep(0.5 * uPx, 3.0 * uPx, abs(d));
  float highlight = line * rim * edge * uIntensity; vec3 color = uBaseColor * base + uLineColor * highlight;
...
export function SpecularButton({
  ...
  useEffect(() => {
    ...
    const onPointerMove = (event: PointerEvent) => {
      ...
      } else pointerAngle = Math.atan2(cy - event.clientY, event.clientX - cx);
    ...
    const update = (now: number) => {
      ...
      bright += (targetBrightness - bright) * (1 - Math.exp(-delta * 8));
      lineColorValue.set(props.lineColor);
      baseColorValue.set(props.baseColor);
      program.uniforms.uAngle.value = angle;
      program.uniforms.uRadius.value = Math.min(props.radius, Math.min(size.w, size.h) / 2) * dpr;
